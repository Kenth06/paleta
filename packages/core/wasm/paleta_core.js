 ...
 * @returns {number}
 */
export function build_histogram_total(rgba, width, height, step, alpha_threshold, include_white) {
    const ptr0 = passArray8ToWasm0(rgba, wasm.__wbindgen_export);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.build_histogram_total(ptr0, len0, width, height, step, alpha_threshold, include_white);
    return ret >>> 0;
}
 ...
 * @returns {Uint8Array}
 */
export function quantize_wu(rgba, width, height, count, step, alpha_threshold, include_white) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passArray8ToWasm0(rgba, wasm.__wbindgen_export);
        const len0 = WASM_VECTOR_LEN;
        wasm.quantize_wu(retptr, ptr0, len0, width, height, count, step, alpha_threshold, include_white);
        var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
        var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
        var v2 = getArrayU8FromWasm0(r0, r1).slice();
        wasm.__wbindgen_export2(r0, r1 * 1, 1);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('paleta_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
